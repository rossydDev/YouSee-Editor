...
import { Plus, Trash2, FileText, Clock, Clapperboard, Layers } from "lucide-react";
...
export default function Dashboard() {
  ...
  // 1. Filtra roteiros que TEM série
  ...
  // 2. Filtra roteiros que NÃO tem série (Soltos)
  const looseScripts = scripts.filter(s => !s.seriesTitle?.trim());

  // 3. Ordena os capítulos dentro das séries
  Object.keys(seriesGroups).forEach(series => {
    seriesGroups[series].sort((a, b) => (a.chapterNumber || 0) - (b.chapterNumber || 0));
  });
      ...
      <main className="max-w-6xl mx-auto space-y-16">
        ...
        {scripts.length === 0 && (
          <div className="flex flex-col items-center justify-center py-32 border-2 border-dashed border-zinc-800 rounded-3xl bg-zinc-900/30">
            ...
            <h2 className="text-2xl font-bold text-white mb-2">Sua estante está vazia</h2>
            <p className="text-zinc-500 text-lg mb-8 max-w-md text-center">
              Comece a escrever sua próxima obra-prima agora mesmo. O editor salva tudo automaticamente.
            </p>
            <button
              onClick={handleCreateNew}
          ...
          <section key={seriesName} className="animate-fade-in">
            <div className="flex items-center gap-3 mb-6 border-b border-zinc-800 pb-4">
              ...
              <span className="text-sm text-zinc-500 bg-zinc-900 px-2 py-1 rounded-full">
                {seriesScripts.length} {seriesScripts.length === 1 ? 'capítulo' : 'capítulos'}
              </span>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {seriesScripts.map((script) => (
                <ScriptCard key={script.id} script={script} onDelete={handleDelete} isSeriesItem />
              ))}
            </div>
          </section>
        ))}
...
// --- COMPONENTE DO CARD (Para evitar repetição) ---
function ScriptCard({ script, onDelete, isSeriesItem }: { script: Script, onDelete: any, isSeriesItem?: boolean }) {
  return (
    <Link
    ...
    >
      <div className="flex justify-between items-start mb-4">
        {isSeriesItem ? (
          ...
          <div className="flex flex-col">
            <span className="text-xs text-orange-500 font-bold uppercase tracking-wider mb-1">Capítulo</span>
      ...
      {/* Mostra a série no card solto caso tenha bugado ou para contexto */}
